...
function () {
  ...
  _createClass(Editor, [{
    ...
    value: function mount(editorNode) {
      ...
      contentEditableNode.addEventListener("input", this.onInputChange, false); //ContentEditableNode starts to listen on every event defined in eventsAndCbs
      ...
      for (var func in this.toolbarOptions) {
        if (this.toolbarOptions[func].status) {
          ...
          var item = opt.name; //item = "B", "I", "RA", etc.
...
function (_HTMLElement) {
  _inherits(RichTextEditor, _HTMLElement);
